 ...
 *  MANUAL - /linear afk-compress start; runs until all files done or stopped.
 *
...
public final class IdleRecompressor {
            ...
            while (!Thread.currentThread().isInterrupted()) {
                ...
                if (idleMs >= idleThresholdMs()) {
                    LinearRuntime.LOGGER.info(
                            "[Linear] Server idle for {} min - starting background recompression.",
                            idleMs / 60_000L);
                    startWorker(false);
                }
            }
    ...
    private static void logCompletion() {
        ...
        if (total == 0) {
            LinearRuntime.LOGGER.info("[Linear] Recompression done: no .linear files found.");
            return;
        }

        StringBuilder msg = new StringBuilder("[Linear] Recompression done: ")
    ...
    private static void doRecompression() {
        for (Path folder : KNOWN_FOLDERS) {
            ...
            try (Stream<Path> s = Files.list(folder)) {
                ...
                LinearRuntime.LOGGER.warn("[Linear] Cannot list {}: {}",
                ...
                try {
                    ...
                    if (result.outcome() == RecompressOutcome.UPGRADED) {
                        LinearRuntime.LOGGER.debug(
                                "[Linear] Recompressed {} - saved {} bytes.",
                    ...
                    LinearRuntime.LOGGER.warn("[Linear] Recompression failed for {}: {}",
    ...
    private static void awaitHeapHeadroom(Path path) throws InterruptedException {
        ...
        while (availableHeapPercent() < thresholdPercent) {
            ...
            LinearRuntime.LOGGER.info(
                    "[Linear] Pausing recompression for {} because JVM heap headroom is {}% (< {}%).",
